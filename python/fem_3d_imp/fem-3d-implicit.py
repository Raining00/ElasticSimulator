...
class Object:
    def __init__(self, filename, index_start=1):
        ...
        self.ndc = ti.Vector.field(self.dim+1, dtype=ti.f32, shape=self.vn)

        self.center = ti.Vector.field(self.dim, dtype=ti.f32, shape=())
        self.lowerCorner = ti.Vector.field(self.dim, dtype=ti.f32, shape=())
        self.higherCorner = ti.Vector.field(self.dim, dtype=ti.f32, shape=())
        ...
        # df/dx
        self.K = ti.field(dtype=ti.f32, shape=(self.dim*self.vn, self.dim*self.vn))
        ...
        self.x = ti.field(dtype=ti.f32, shape=self.dim*self.vn) # speed at next time
        self.x_new = ti.field(dtype=ti.f32, shape=self.dim*self.vn)
        self.r = ti.field(dtype=ti.f32, shape=self.dim*self.vn) # residual
        self.d = ti.field(dtype=ti.f32, shape=self.dim*self.vn)
        self.q = ti.field(dtype=ti.f32, shape=self.dim*self.vn)

        # for rendering
        self.color = ti.field(dtype=ti.i32, shape=self.vn)
...
while gui.running and not gui.get_event(gui.ESCAPE):
    ...
    obj.la = obj.E * obj.nu / ((1 + obj.nu) * (1 - 2 * obj.nu))
    ...
    for i in range(obj.fn):
        for j in range(3):
            if abs(obj.ndc[obj.face[i][j]].z) < 1 and abs(obj.ndc[obj.face[i][(j+1)%3]].z) < 1:
                gui.line(obj.ndc[obj.face[i][j]], obj.ndc[obj.face[i][(j+1)%3]])
